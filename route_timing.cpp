...
bool try_timing_driven_route_tmpl(const t_router_opts& router_opts,
    ...
    //Py_Initialize( );
 //PyRun_SimpleString("from home.umair.vtr.vpr import data"); /* load .py file */
 //PyRun_SimpleString("t1= np.loadtxt('file1.txt'); print(t1) "); /* on python path */
    ...
    auto& grid1 = g_vpr_ctx.device().grid;
    //int max_grid_dim1 = std::max(grid1.width(), grid1.height());
    std::cout<<"Grid dimensions " << grid1.width() <<", "<< grid1.height() << std::endl;
    ...
    //std::cout <<"skipping routing the net "<< std::endl;	
    /*
    ...
    std::vector<float> overfl;

    print_route_status_header();
    int mxit= router_opts.max_router_iterations;
    mxit= 100;
    for (itry = 1; itry <= mxit; ++itry) {
	std::cout<<"Router iteration #: "<<itry<< std::endl;
	...
	float rr1= float(overuse_info.overused_nodes);          //overused_node_ratio();
	overfl.push_back(rr1);
...
std::cout<<"size of overfl is "<<overfl.size() << std::endl;
ofstream MyFile("overinfo.txt");
for (float v1: overfl)
{
float v2= v1 * 100;
MyFile << v2 <<"\n";
...
bool timing_driven_route_net(ConnectionRouter& router,
    ...
    //pName = PyUnicode_FromString((char*)"person");
    ...
    //std::cout << "Net delays of the pins of a net are as follows " << std::endl;
    //for (unsigned ipin = 1; ipin < cluster_ctx.clb_nlist.net_pins(net_id).size(); ++ipin) {
    //    std::cout << net_delay[ipin] << ",";
